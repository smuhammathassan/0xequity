...
async function main() {
        ...
        let ERC3643 = await ethers.getContractFactory("Token");
        ...
        let FactoryInstance = await Factory.deploy(IAInstance.address, propetyBytecode);
        ...
        let TokenDetails = ethers.utils.keccak256((accounts[0].address, "Texas Benglos", "TB", 0, IdentitiyInstance.address, zeroAddress, [accounts[0].address, FactoryInstance.address], [accounts[0].address, FactoryInstance.address], [accounts[0].address, FactoryInstance.address], '0x00'));
