...
describe.only("ERC3643", function () {
  ...
  before("NEWSETUP: Deploying factory ", async function () {
    ...
    JUSDC = await _deploy("MintableBurnableSyntheticTokenPermit", [
      "jUSDC",
      "jUSDC",
      6,
    ]);
    ...
    JEuro = await _deploy("MintableBurnableSyntheticTokenPermit", [
      "jEUR",
      "jEUR",
      18,
    ]);
    ...
    jTry = await _deploy("MintableBurnableSyntheticTokenPermit", [
      "jTRY",
      "jTRY",
      18,
    ]);
    ...
    vTRY = await _deploy("MintableBurnableSyntheticTokenPermit", [
      "vTRY",
      "vTRY",
      18,
    ]);
    ...
    RShareInstance = await _deployWithLibrary("RentShare", RShare, [
      vTRY.address,
    ]);

    const tx122921 = await vTRY
      .connect(tokeny)
      .addMinter(RShareInstance.address);
    await tx122921.wait();
    ...
    mock1 = await _deploy("MockRandomAggregator", [
      ethers.utils.parseUnits("0.05332091", 8),
      1,
    ]);
    console.log("mock1 Address : ", mock1.address);

    mock2 = await _deploy("MockRandomAggregator", [
    ...
    mock3 = await _deploy("MockRandomAggregator", [
    ...
    const burnerRole = ethers.utils.keccak256(
      ethers.utils.toUtf8Bytes("Burner")
    );
    const RewardTokenInterface =
      ethers.utils.formatBytes32String("RewardToken");
    ...
    console.log("Claim IssuerContract address : ", claimIssuerContract.address);
    ...
    const identityRegistryAddressA = await TOOOOKENN.connect(
      tokeny
    ).identityRegistry();
    ...
    await RShareInstance.connect(tokeny).updateRewardPerMonth(
      symbol,
      ethers.utils.parseUnits("500", 18)
    );

    //----------------------------------------------------------------------------------------
    rentDistributor = await _deploy("RentDistributor", [
      vTRY.address,
      jTry.address,
    ]);
    ...
    const tx1120 = await jTry
      .connect(tokeny)
      .mint(rentDistributor.address, ethers.utils.parseUnits("1000000000", 18));
    await tx1120.wait();
  ...
  it("anyone => f(BUY)", async function () {
    ...
    const BeforeStableUserBalance = await jTry
      .connect(tokeny)
    ...
    const AfterStablUsereBalance = await jTry
      .connect(tokeny)
      .balanceOf(user2.address);
    console.log("After Stable User TOkens  =>", AfterStablUsereBalance);

    const AfterStableMarketplaceBalance = await jTry
      .connect(tokeny)
    ...
    const PropertyBalance = await WLegalTokenAddessInstance.connect(
      tokeny
    ).balanceOf(user2.address);
  ...
  it("anyone => f(SELL)", async function () {
    ...
    const check = await priceFeed
      .connect(tokeny)
      .getCurrencyToFeed(jTry.address);
  ...
  it("anyone => f(BUY) diffrent priceFeeds", async function () {
    ...
    const JEuroUser2Balance = await JEuro.connect(tokeny).balanceOf(
      user2.address
    );
    console.log("JEuro User Tokens  =>", JEuroUser2Balance);

    const JEuroMarketplaceBalance = await JEuro.connect(tokeny).balanceOf(
      Marketplace.address
    );
    console.log(
  ...
  it("anyone => f(BUY) via 6 decimal", async function () {
    ...
    const JUSDCUser2Balance = await JUSDC.connect(tokeny).balanceOf(
      user2.address
    );
    console.log(
      "JUSDC User Tokens  =>",
      ethers.utils.formatUnits(JUSDCUser2Balance, 6)
    );

    const JUSDCMarketplaceBalance = await JUSDC.connect(tokeny).balanceOf(
      Marketplace.address
    );
    console.log(
  ...
  it("revoke SBT => revert", async function () {
    ...
    await expect(
      Marketplace.connect(user2).swap([JUSDC.address, WLegalTokenAddess, 10])
  ...
  it("Non KYC user can't receive => revert", async function () {
    ...
    await expect(
      WLegalTokenAddessInstance.connect(user2).transfer(user1.address, 10)
    ).to.be.revertedWithCustomError(WLegalTokenAddessInstance, "NonKYC");
  });
  ...
  it("Giving same community access twice => revert", async function () {
    await expect(
      SBT.connect(tokeny).mint(user1.address, "0xEquity")
    ).to.be.revertedWithCustomError(SBT, "CantMintTwice");
  });

  it("Tring to add community that is not approved => revert", async function () {
    await expect(
      SBT.connect(tokeny).mint(user1.address, "testCommunity")
    ).to.be.revertedWithCustomError(SBT, "WrongCommunityName");
  });

  it("Tring to add community that is not approved mintBatch => revert", async function () {
    await expect(
      SBT.connect(tokeny).mintBatch(agent.address, [
        "0xEquity",
        "testCommunity",
      ])
    ).to.be.revertedWithCustomError(SBT, "WrongCommunityName");
  ...
  it("Tring to approve already approved property => revert", async function () {
    await expect(
      SBT.connect(tokeny).addCommunity("0xEquity", 2)
    ).to.be.revertedWithCustomError(SBTLib, "AlreadyApprovedCommunity");
  });
  ...
  it("mintbatch", async function () {
    ...
    let AddingCommunity = await SBT.connect(tokeny).addCommunity(
      "IdeoFuzion",
      2
    );
    await AddingCommunity.wait();

    let mintingBatch = await SBT.connect(tokeny).mintBatch(user1.address, [
      "0xEquity",
      "IdeoFuzion",
    ]);
    await mintingBatch.wait();
    let balance1 = await SBT.connect(tokeny).getBalanceOf(
      user1.address,
      "0xEquity"
    );
    let balance2 = await SBT.connect(tokeny).getBalanceOf(
      user1.address,
      "IdeoFuzion"
    );
  ...
  it("Should not be able to buy with non approved Community against wrapped token => revert", async function () {
    ...
    await expect(
      Marketplace.connect(user1).swap([JUSDC.address, WLegalTokenAddess, 10])
    ).to.be.revertedWithCustomError(WLegalTokenAddessInstance, "NonKYC");
  });

  it("Should not be able to approve same community against legal  => revert", async function () {
    await expect(
      SBT.connect(tokeny).addApprovedCommunity("WXEFR1", "0xEquity")
    ).to.be.revertedWithCustomError(SBTLib, "AlreadyApprovedCommunity");
  });

  it("Should be able to buy with approved Community", async function () {
    let addingNewCommunity = await SBT.connect(tokeny).addApprovedCommunity(
      "WXEFR1",
      "IdeoFuzion"
    );
    await addingNewCommunity.wait();

    let minting = await SBT.connect(tokeny).mint(
      Marketplace.address,
      "IdeoFuzion"
    );
    await minting.wait();
  ...
  it("getCommunityToId wrong community name => revert ", async function () {
    await expect(
      SBT.connect(tokeny).getCommunityToId("0xEquity1")
    ).to.be.revertedWithCustomError(SBTLib, "CommunityDoesnotExist");
  });
  ...
  it("DoesCommunityExist ", async function () {
    ...
    await expect(
      SBT.connect(user1).DoesCommunityExist("0xEquity")
    ).to.be.revertedWith(
  ...
  it("removeCommunity => revert CommunityDoesnotExist ", async function () {
    await expect(
      SBT.connect(tokeny).removeCommunity("0xEquity1")
    ).to.be.revertedWithCustomError(SBTLib, "CommunityDoesnotExist");
  });
  ...
  it("bulkApproveCommunities  ", async function () {
    ...
    await SBT.connect(tokeny).bulkApproveCommunities("WXEFR1", [
      "Fins",
      "Brits",
    ]);
    let finsExist = await SBT.connect(tokeny).getApprovedSBT("WXEFR1", "Fins");
    let britsExist = await SBT.connect(tokeny).getApprovedSBT(
      "WXEFR1",
      "Brits"
    );
  ...
  it("bulkRemoveCommunities  ", async function () {
    await SBT.connect(tokeny).bulkAddCommunities(
      ["Perssuian", "Sweeds", "Italians", "Scots"],
      [5, 6, 7, 8]
    );
    ...
    let SweedsExist = await SBT.connect(tokeny).getApprovedSBT(
      "WXEFR1",
      "Sweeds"
    );
    let ScotsExist = await SBT.connect(tokeny).getApprovedSBT(
      "WXEFR1",
      "Scots"
    );
    ...
    await SBT.connect(tokeny).bulkRemoveCommunities("WXEFR1", [
      "Perssuian",
      "Sweeds",
      "Italians",
      "Scots",
    ...
    let SweedsExist2 = await SBT.connect(tokeny).getApprovedSBT(
      "WXEFR1",
      "Sweeds"
    );
    let ScotsExist2 = await SBT.connect(tokeny).getApprovedSBT(
      "WXEFR1",
      "Scots"
    );
    let ItaliansExist2 = await SBT.connect(tokeny).getApprovedSBT(
      "WXEFR1",
      "Italians"
    );
    let PerssuianExist2 = await SBT.connect(tokeny).getApprovedSBT(
      "WXEFR1",
      "Italians"
    );
  ...
  it("bulkAdding Again for testing to see they are indeed removed in the last test  ", async function () {
    await SBT.connect(tokeny).mintBatch(Marketplace.address, [
      "Perssuian",
      "Sweeds",
      "Italians",
    ]);
    await SBT.connect(tokeny).revokeBatch(Marketplace.address, [
      "Perssuian",
      "Sweeds",
      "Italians",
    ]);

    let PerssuianBalance = await SBT.connect(tokeny).getBalanceOf(
      Marketplace.address,
      "Perssuian"
    );
    expect(PerssuianBalance).to.be.equals(0);

    let SweedsBalance = await SBT.connect(tokeny).getBalanceOf(
      Marketplace.address,
      "Sweeds"
    );
    expect(SweedsBalance).to.be.equals(0);

    let ItaliansBalance = await SBT.connect(tokeny).getBalanceOf(
      Marketplace.address,
      "Italians"
    );
  ...
  it("supportsInterface ", async function () {
    ...
    await expect(
      SBT.connect(agent).safeTransferFrom(
        agent.address,
        user2.address,
        5,
        1,
